...
# b
print('b:')
rmse_test = (mse(Y_test, reg_train.predict(X_test))) ** 0.5
print("The rmse for testing data is", round(rmse_test, 3))

# c
plt.scatter(Y_test, reg_train.predict(X_test))
plt.xlabel('Actual Rings')
plt.ylabel('Predicted Rings')
...
print('b:')
RMSE = []
Xt = np.array(test['Shell weight']).reshape(-1, 1)
Y_pred = []
for p in P:
    poly_features = PolynomialFeatures(p)  # p is the degree
    x_poly = poly_features.fit_transform(X)
    reg = LinearRegression()
    reg.fit(x_poly, Y_train)
    xt_poly = poly_features.fit_transform(Xt)
    Y_pred = reg.predict(xt_poly)
    rmse = (mse(Y_test, Y_pred)) ** 0.5
    RMSE.append(rmse)
    print("The rmse for p=", p, 'is', round(rmse, 3))
    # d
    # because the best degree of polynomial is 4 as p=4 has minimum rmse
    if p == 4:
...
# c
# because p=4 has the lowest rmse
Xt = np.array(train['Shell weight']).reshape(-1, 1)
xt_poly = PolynomialFeatures(4).fit_transform(Xt)
xp_poly = PolynomialFeatures(4).fit_transform(x)
reg = LinearRegression()
...
for p in P:
    ...
    print("The rmse for p=", p, 'is', round(rmse, 3))
    # c
    # because the best degree of polynomial is 2 as p=2 has minimum rmse
    if p == 2:
        plt.scatter(Y_test, Y_pred)
        plt.xlabel('Actual Rings')
        plt.ylabel('Predicted Rings')
